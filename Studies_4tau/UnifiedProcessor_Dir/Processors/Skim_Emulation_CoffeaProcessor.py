...
def weight_calc(sample,numEvents=1):
	...
	return Lumi_2016*xSection_Dictionary[sample]/numEvents
...
class PlottingScriptProcessor(processor.ProcessorABC):
	...
	def process(self, events):
		...
		muon = ak.zip(
			{
				...
				"IDSelec_Tight": events.Muon_tightId,
				"D0": events.Muon_dxy,
				"Dz": events.Muon_dz,
		...
		h_boostedtau_pT_Trigger = hist.Hist.new.Regular(20,0,600,label = r"Boosted $\tau$ $p_T$ [GeV]",overflow = True).Weight()
		h_Leadingboostedtau_pT_Trigger = hist.Hist.new.Regular(20,0,600,label = r"Boosted $\tau$ Leading $p_T$ [GeV]",overflow = True).Weight()
		h_Subleadingboostedtau_pT_Trigger = hist.Hist.new.Regular(20,0,600,label = r"Boosted $\tau$ Subleading $p_T$ [GeV]",overflow = True).Weight()
		h_Thirdleadingboostedtau_pT_Trigger = hist.Hist.new.Regular(20,0,600,label = r"Boosted $\tau$ 3rd-leading $p_T$ [GeV]",overflow = True).Weight()
		h_Fourthleadingboostedtau_pT_Trigger = hist.Hist.new.Regular(20,0,600,label = r"Boosted $\tau$ 4th-leading $p_T$ [GeV]",overflow = True).Weight()
		h_boostedtau_eta_Trigger = hist.Hist.new.Regular(20,-4,4,label = r"Boosted $\tau$ $\eta$",overflow = True).Weight()
		h_boostedtau_phi_Trigger = hist.Hist.new.Regular(20,-pi,pi,label = r"Boosted$\tau$ $\phi$",overflow = True).Weight()
		h_boostedtau_raw_iso_Trigger = hist.Hist.new.Regular(20,-1,1,label=r"Raw MVA Score",overflow = True).Weight()
		
		#Basic Kinematic histograms of tau/HPS tau
		h_tau_pT_Trigger = hist.Hist.new.Regular(20,0,600,label = r" $\tau$ $p_T$ [GeV]",overflow = True).Weight()
		h_Leadingtau_pT_Trigger = hist.Hist.new.Regular(20,0,600,label = r"$\tau$ Leading $p_T$ [GeV]",overflow = True).Weight()
		h_tau_eta_Trigger = hist.Hist.new.Regular(20,-4,4,label = r" $\tau$ $\eta$",overflow = True).Weight()
		h_tau_phi_Trigger = hist.Hist.new.Regular(20,-pi,pi,label = r"$\tau$ $\phi$",overflow = True).Weight()
		
		#Basic Kinematic histograms leptons (muons and electrons)
		h_electron_pT_Trigger = hist.Hist.new.Regular(15,0,300,label = r"e $p_T$ [GeV]",overflow = True).Weight()
		h_Leadingelectron_pT_Trigger = hist.Hist.new.Regular(15,0,300,label = r"e Leading $p_T$ [GeV]",overflow = True).Weight()
		h_electron_eta_Trigger = hist.Hist.new.Regular(20,-4,4,label = r"e $\eta$",overflow = True).Weight()
		h_electron_phi_Trigger = hist.Hist.new.Regular(20,-pi,pi,label = r"e Leading $\phi$",overflow = True).Weight()
		h_muon_pT_Trigger = hist.Hist.new.Regular(15,0,600,label = r"$\mu$ $p_T$ [GeV]",overflow = True).Weight()
		#h_Leadingmuon_pT_Trigger = hist.Hist.new.Regular(15,0,600,label = r"$\mu$ Leading $p_T$ [GeV]",overflow = True).Weight()
		h_Leadingmuon_pT_Trigger = hist.Hist.new.Regular(40,0,400,label = r"$\mu$ Leading $p_T$ [GeV]",overflow = True).Weight()
		h_muon_eta_Trigger = hist.Hist.new.Regular(20,-4,4,label = r"$\mu$ $\eta$",overflow = True).Weight()
		h_Leadingmuon_eta_Trigger = hist.Hist.new.Regular(20,-4,4,label = r"$\mu$ $\eta$",overflow = True).Weight()
		h_muon_phi_Trigger = hist.Hist.new.Regular(20,-pi,pi,label = r"$\mu$ $\phi$",overflow = True).Weight()
		
		#Basic Kinematic histograms Jets (check which Jets most useful based on 
		h_Jet_pT_Trigger = hist.Hist.new.Regular(50,0,700,label = r"Jet $p_T$ [GeV]",overflow = True).Weight()
		h_LeadingJet_pT_Trigger = hist.Hist.new.Regular(50,0,700,label = r"Jet Leading $p_T$ [GeV]",overflow = True).Weight()
		h_Jet_eta_Trigger = hist.Hist.new.Regular(20,-4,4,label = r"Jet $\eta$",overflow = True).Weight()
		h_Jet_phi_Trigger = hist.Hist.new.Regular(20,-pi,pi,label = r"Jet $\phi$",overflow = True).Weight()
		h_AK8Jet_pT_Trigger = hist.Hist.new.Regular(50,0,700,label = r"AK8Jet $p_T$ [GeV]",overflow = True).Weight()
		h_LeadingAK8Jet_pT_Trigger = hist.Hist.new.Regular(50,0,700,label = r"AK8Jet Leading $p_T$ [GeV]",overflow = True).Weight()
		h_AK8Jet_eta_Trigger = hist.Hist.new.Regular(20,-4,4,label = r"AK8Jet $\eta$",overflow = True).Weight()
		h_AK8Jet_phi_Trigger = hist.Hist.new.Regular(20,-pi,pi,label = r"AK8Jet $\phi$",overflow = True).Weight()
		
		#Add MET, HT and MHT histogram
		h_MET_Trigger = hist.Hist.new.Regular(20,0,500,label=r"MET [GeV]",overflow = True).Weight()
		h_HT_Trigger = hist.Hist.new.Regular(40,0,1200,label=r"HT [GeV]",overflow = True).Weight()
		h_MHT_Trigger = hist.Hist.new.Regular(20,0,500,label=r"MHT [GeV]",overflow = True).Weight()

		#Add cutflow and N-1 tables
		if (self.ApplyTrigger):
			h_CutFlow = hist.Hist.new.StrCategory(["SkimOnly","Trigger","METCut","nFatJetReq","FlagReq","PVSelec","LeadingBoostedTau","SubleadingBoostedTau","3rdLeadingBoostedTau","4thLeadingBoostedTau"]).Double()
			h_NMinus1 = hist.Hist.new.StrCategory(["SkimOnly","Trigger","METCut","nFatJetReq","FlagReq","PVSelec","LeadingBoostedTau","SubleadingBoostedTau","3rdLeadingBoostedTau","4thLeadingBoostedTau"]).Double()
		else:
	...
	#	event_level["MET_Phi"]= METPhiCorrections["MET_phi_corr"]
		...
		if (self.ApplyTrigger):
			...
			n_HLT = int(ak.num(event_level.Mu_Trigger,axis=0))
			...
			n_nMuon = int(ak.num(event_level.Mu_Trigger,axis=0))

			#pT, eta, ID and isolation selection updated
			mu_selec_Mask = (
						(muon.pt > 52) &
						(abs(muon.eta) < 2.4) &
						(muon.IDSelec_Tight) &
			...
			#mu_selec_cond = np.bitwise_and(muon.pt > 52,np.bitwise_and(np.abs(muon.eta) < 2.4, np.bitwise_and(muon.IDSelec_Tight, muon.RelIso < 0.15))) #Requre there to exist a muon that passes these selections
			#mu_selec_cond = ak.any(mu_selec_cond,axis=1)
			
			tau = tau[mu_selec_cond]
			...
			n_OfflineMu = int(ak.num(event_level.Mu_Trigger,axis=0))
			...
			h_NMinus1.fill("Trigger",weight=n_Skim - n_Trigger)
		else:
			n_Trigger = 0
			n_HLT = 0
			n_nMuon = 0
		...
		if (self.nBoostedTau_Selec > 0):
			...
			decayMode_Cond = boostedtau.decay
			...
			h_NMinus1.fill("LeadingBoostedTau",weight=n_preBoostedTau - n_LeadBoostedTau)

			n_PreCuts = n_LeadBoostedTau				
			...
			if (self.nBoostedTau_Selec > 1):
				...
				tau = tau[sublead_boostedtau_cond]
			...
			if (self.nBoostedTau_Selec > 2):
				...
				tau = tau[thirdlead_boostedtau_cond]
			...
			if (self.nBoostedTau_Selec > 3):
				...
				tau = tau[fourthlead_boostedtau_cond]
		...
		h_NMinus1.fill("METCut",weight=n_PreCuts - n_MET)	
