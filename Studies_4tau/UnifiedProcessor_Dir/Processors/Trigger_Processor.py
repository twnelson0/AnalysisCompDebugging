...
class PlottingScriptProcessor(processor.ProcessorABC):
	...
	def process(self, events):
		...
		h_MET_Trigger = hist.Hist.new.Regular(50,0,500,label=r"MET [GeV]").Double()
		h_HT_Trigger = hist.Hist.new.Regular(40,0,1200,label=r"HT [GeV]").Double()
		h_MHT_Trigger = hist.Hist.new.Regular(50,0,500,label=r"MHT [GeV]").Double()
		...
		if (self.ApplyTrigger):
			#HLT Trigger(s)
			trigger_cond = event_level.MET_trigger1
			...
			#MET and MHT selections
			tau = tau[event_level.MET_pt > 130]
			boostedtau = boostedtau[event_level.MET_pt > 130]
			AK8Jet = AK8Jet[event_level.MET_pt > 130]
			Jet = Jet[event_level.MET_pt > 130]
			electron = electron[event_level.MET_pt > 130]
			muon = muon[event_level.MET_pt > 130]
			event_level = event_level[event_level.MET_pt > 130]	
			
			#MHT selections
			tau = tau[event_level.MHT > 130]
			boostedtau = boostedtau[event_level.MHT > 130]
			AK8Jet = AK8Jet[event_level.MHT > 130]
			Jet = Jet[event_level.MHT > 130]
			electron = electron[event_level.MHT > 130]
