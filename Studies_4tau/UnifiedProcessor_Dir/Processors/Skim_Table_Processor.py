...
class TableProcessor(processor.ProcessorABC):
	...
	def process(self, events):
		...
		return{
			dataset: {
				...
				"SumGenWeight": np.sum(event_level.event_weight),
				"Event_Count": int(ak.num(event_level.event_weight,axis=0)),
				"Yield": np.sum(ak.to_list(event_level.event_weight*CrossSec_Weight)),
				"n_Skim": n_Skim,
				"n_MET": n_MET,
				"n_FatJet": n_FatJet,
