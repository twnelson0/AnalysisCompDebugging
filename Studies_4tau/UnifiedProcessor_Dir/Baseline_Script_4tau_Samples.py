...
import glob
import json
from Processors import Simple_Tau_Selec_Only_Proc as TauProcessor
...
#				)
	...
	if (run_on_condor):
		...
		#Pass modules to HTC
		cloudpickle.register_pickle_by_value(TauProcessor)
	...
	for n_taus in range(4,5):
		...
		start_time = time.time()
		fourtau_out = runner(file_dict, treename="Events", processor_instance=TauProcessor.PlottingScriptProcessor(sumWEvents_Dict = sumWEvents_Dict, nBoostedTaus = n_taus, ApplyTrigger = True)) #Modified for NanoAOD (changd treename)
		end_time = time.time()
		...
		outfile = os.path.join(os.getcwd() + "/Output_4Tau/", f"output_{n_taus}_boosted_tau_selec_Control.coffea")
