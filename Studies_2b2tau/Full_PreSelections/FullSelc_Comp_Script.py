...
#		del event_level_1, event_level_2, event_level_3
		...
		tau_hps = tau[np.bitwise_and(ak.num(tau,axis=1) > 1, ak.num(boostedtau,axis=1) < 2)]
		muon_hps = muon[np.bitwise_and(ak.num(tau,axis=1) > 1, ak.num(boostedtau,axis=1) < 2)]
		electron_hps = electron[np.bitwise_and(ak.num(tau,axis=1) > 1, ak.num(boostedtau,axis=1) < 2)]
		AK8Jet_hps = AK8Jet[np.bitwise_and(ak.num(tau,axis=1) > 1, ak.num(boostedtau,axis=1) < 2)]
		Jet_hps = Jet[np.bitwise_and(ak.num(tau,axis=1) > 1, ak.num(boostedtau,axis=1) < 2)]
		event_level_hps = event_level[np.bitwise_and(ak.num(tau,axis=1) > 1, ak.num(boostedtau,axis=1) < 2)]
		boostedtau_hps = boostedtau[np.bitwise_and(ak.num(tau,axis=1) > 1, ak.num(boostedtau,axis=1) < 2)]
		
		tau_boost = tau[np.bitwise_and(ak.num(boostedtau,axis=1) > 1, ak.num(tau,axis=1) < 2)]
		muon_boost = muon[np.bitwise_and(ak.num(boostedtau,axis=1) > 1, ak.num(tau,axis=1) < 2)]
		electron_boost = electron[np.bitwise_and(ak.num(boostedtau,axis=1) > 1, ak.num(tau,axis=1) < 2)]
		AK8Jet_boost = AK8Jet[np.bitwise_and(ak.num(boostedtau,axis=1) > 1, ak.num(tau,axis=1) < 2)]
		Jet_boost = Jet[np.bitwise_and(ak.num(boostedtau,axis=1) > 1, ak.num(tau,axis=1) < 2)]
		event_level_boost = event_level[np.bitwise_and(ak.num(boostedtau,axis=1) > 1, ak.num(tau,axis=1) < 2)]
		boostedtau_boost = boostedtau[np.bitwise_and(ak.num(boostedtau,axis=1) > 1, ak.num(tau,axis=1) < 2)]
		
		tau_both = tau[np.bitwise_and(ak.num(boostedtau,axis=1) > 1,ak.num(tau,axis=1) > 1)]
		muon_both = muon[np.bitwise_and(ak.num(boostedtau,axis=1) > 1,ak.num(tau,axis=1) > 1)]
		electron_both = electron[np.bitwise_and(ak.num(boostedtau,axis=1) > 1,ak.num(tau,axis=1) > 1)]
		AK8Jet_both = AK8Jet[np.bitwise_and(ak.num(boostedtau,axis=1) > 1,ak.num(tau,axis=1) > 1)]
		Jet_both = Jet[np.bitwise_and(ak.num(boostedtau,axis=1) > 1,ak.num(tau,axis=1) > 1)]
		event_level_both = event_level[np.bitwise_and(ak.num(boostedtau,axis=1) > 1,ak.num(tau,axis=1) > 1)]
		boostedtau_both = boostedtau[np.bitwise_and(ak.num(boostedtau,axis=1) > 1,ak.num(tau,axis=1) > 1)]
		...
		boostedtau_h_cond = np.bitwise_and(boostedtau_h_dR_cond, boostedtau_h_mass_cond)
		...
		#Both taus
		boostedtau_h_dR_cond_both = deltaR_Selec(boostedtau_both[:,0],boostedtau_both[:,1],1.5)
		boostedtau_h_mass_cond_both = dimass_Selec(boostedtau_both[:,0],boostedtau_both[:,1],20)
		boostedtau_h_cond_both = np.bitwise_and(boostedtau_h_dR_cond_both, boostedtau_h_mass_cond_both)
		
		tau_h_dR_cond_both = deltaR_Selec(tau_both[:,0],tau_both[:,1],1.5) 
		tau_h_mass_cond_both = dimass_Selec(tau_both[:,0],tau_both[:,1],20)
		tau_h_cond_both = np.bitwise_and(tau_h_dR_cond_both, tau_h_mass_cond_both)
		
		tau_both = tau_both[np.bitwise_or(boostedtau_h_cond_both, tau_h_cond_both)]
		muon_both = muon_both[np.bitwise_or(boostedtau_h_cond_both, tau_h_cond_both)]
		electron_both = electron_both[np.bitwise_or(boostedtau_h_cond_both, tau_h_cond_both)]
		AK8Jet_both = AK8Jet_both[np.bitwise_or(boostedtau_h_cond_both, tau_h_cond_both)]
		Jet_both = Jet_both[np.bitwise_or(boostedtau_h_cond_both, tau_h_cond_both)]
		event_level_both = event_level_both[np.bitwise_or(boostedtau_h_cond_both, tau_h_cond_both)]
		boostedtau_both = boostedtau_both[np.bitwise_or(boostedtau_h_cond_both, tau_h_cond_both)]
		
		#Set up the hadronic channel
		boostedtau_h = ak.concatenate([boostedtau_hps,ak.concatenate([boostedtau_boost,boostedtau_both])])
		tau_h = ak.concatenate([tau_hps,ak.concatenate([tau_boost,tau_both])])
		electron_h = ak.concatenate([electron_hps,ak.concatenate([electron_boost,electron_both])])
		muon_h = ak.concatenate([muon_hps,ak.concatenate([muon_boost,muon_both])])
